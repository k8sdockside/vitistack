...
(() => {
  ...
  sdk.ready().then(async (ctx) => {
    ...
    function draw() {
      if (!model) return;
      const m = isVM ? model.machines.find((x) => x.vm && x.vm.metadata.name === obj.name && (x.vm.metadata.namespace ?? "") === obj.namespace) : model.machines.find((x) => x.namespace === obj.namespace && x.name === obj.name);
      if (!m) {
        root.replaceChildren(
          isVM ? empty("The Machine this VM was made for is gone.", "It carries vitistack.io/source-machine, and no Machine of that name exists -- it was probably left behind.", "alert") : empty("This machine is not in the model.", "It may have just been created, or deleted.", "machine")
        );
        return;
      }
