...
(() => {
  ...
  function upgradeCard(plan, ctx) {
    ...
    const head2 = el("div", "upcard-head");
    head2.appendChild(icon(plan.verdict === "available" ? "upgrade" : toneIcon(plan.tone), "upcard-ico"));
    const words = el("div", "upcard-words");
    add(words, el("div", "upcard-title", plan.headline), el("div", "upcard-detail", plan.detail));
    head2.appendChild(words);
    ...
    if (plan.verdict === "failed" || plan.verdict === "blocked") {
      if (ctx.write) actions2.push(controls(plan, ctx));
    } else if (!plan.inFlight && (plan.offers.talos.length || plan.offers.kubernetes.length || plan.path.length)) {
      if (ctx.write) actions2.push(offerButtons(plan, ctx));
      const first = plan.path.find((s) => s.exact);
      if (first && (!ctx.write || !ctx.compact)) actions2.push(codeLine(kubectlFor(c, first.kind, first.to), "The same, from a terminal"));
    }
    if (c.upgrade.talos.target && !plan.inFlight && plan.verdict !== "failed" && ctx.write) {
      actions2.push(button(`Clear Talos target ${c.upgrade.talos.targetText}`, "ghost small", "close", () => void askControl(c, "clear-talos-target", ctx)));
    }
    if (actions2.length) add(card2, add(el("div", "upcard-actions"), ...actions2));
  ...
  function machineCell(m, onClick) {
  ...
  function poolRow(pool, onMachine) {
  ...
  function networkBox(n, name, guessed) {
  ...
  function resourceMeters(c) {
    const r = c.resources;
    if (!r) return null;
    const rows = [];
    for (const [label2, use, fmt] of [
      ["CPU", r.cpu, (n) => `${Math.round(n * 10) / 10}`],
      ["Memory", r.memory, bytes],
      ["Disk", r.disk, bytes],
      ["GPU", r.gpu, (n) => String(n)]
    ]) {
      if (!use) continue;
      const cap = quantity(use.capacity);
      const used = quantity(use.used);
      if (!cap) continue;
      rows.push([label2, meter(used ?? 0, cap, { text: `${fmt(used ?? 0)} of ${fmt(cap)}` })]);
    }
  ...
  function clusterDetail(c, ctx) {
    ...
    const onMachine = ctx.openMachine ?? ((m) => void goTo("machines", m.id));
    if (!ctx.compact) {
      root.appendChild(
        head(
          c.name,
    ...
    add(
      ...
      tile({ label: "Kubernetes", value: c.versions.kubernetes ? kubeText(c.versions.kubernetes) : "—", icon: "kubernetes", tone: c.plan?.support?.state === "end-of-life" ? "error" : c.plan?.support?.state === "ending" ? "warn" : void 0 }),
  ...
  function clusterRing(c, size = 64) {
    const counts = /* @__PURE__ */ new Map();
    for (const m of c.machines) counts.set(m.health, (counts.get(m.health) ?? 0) + 1);
    const segs = ["error", "warn", "info", "ok", "muted"].map((t) => ({ value: counts.get(t) ?? 0, tone: t, label: String(t) }));
  ...
  start("clusters", (ctx, root) => {
    ...
    function matches(c) {
      ...
      if (!q) return true;
      return [c.name, c.namespace, c.clusterId, c.environment, c.region, c.zone, c.phase, c.networkName, c.versions.kubernetes ? kubeText(c.versions.kubernetes) : ""].join(" ").toLowerCase().includes(q);
    }
    function draw() {
      if (!model) return;
      const m = model;
      ...
      const by = (t) => m.clusters.filter((c) => c.health === t).length;
      ...
      add(
        ...
        namespaces.length > 1 ? select([{ value: "", label: "All namespaces" }, ...namespaces.map((n) => ({ value: n, label: n }))], state.ns, (v) => {
          state.ns = v;
          save();
          redraw();
        }, "Namespace") : null,
        envs.length > 1 ? select([{ value: "", label: "All environments" }, ...envs.map((n) => ({ value: n, label: n }))], state.env, (v) => {
          state.env = v;
          save();
          redraw();
        }, "Environment") : null,
    ...
    function clusterCard(c) {
      const card2 = el("button", `ccard ${c.health}${state.sel === c.id ? " sel" : ""}`);
      card2.type = "button";
      card2.dataset.focus = "cc:" + c.id;
      card2.setAttribute("aria-label", `${c.name}, ${c.phase || "no status yet"}`);
      ...
      add(
        vers,
        versionPill("kubernetes", c.versions.kubernetes, { tone: support === "end-of-life" ? "error" : support === "ending" ? "warn" : void 0 }),
        c.versions.talos || c.providerType === "talos" ? versionPill("talos", c.versions.talos) : null
      );
      card2.appendChild(vers);
      const meta2 = el("div", "ccard-meta");
      add(
        meta2,
      ...
      if (plan && plan.verdict !== "current" && plan.verdict !== "unknown") {
        const label2 = plan.verdict === "available" ? plan.path[0] ? plan.path[0].kind === "talos" ? talosText(plan.path[0].to) : kubeText(plan.path[0].to) : "upgrade" : plan.verdict.replace("-", " ");
        add(foot, el("span", "push"), chip(label2, plan.tone, "upgrade", plan.headline));
      }
